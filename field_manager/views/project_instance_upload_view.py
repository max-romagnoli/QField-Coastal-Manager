...
from field_manager.models import ProjectInstance, ProjectInstanceUploadRecord, ProjectInstanceGeoJSONLayer
...
class ProjectInstanceUploadView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if not instance_slug:
            return Response({"error": "No 'instance_slug' specified."}, status=status.HTTP_400_BAD_REQUEST)
        try:
            instance = ProjectInstance.objects.get(instance_slug=instance_slug)
        except ProjectInstance.DoesNotExist:
            return Response({"error": f"No ProjectInstance found with slug {instance_slug}."}, status=status.HTTP_404_NOT_FOUND)
        uploaded_files = request.FILES.getlist("file")
        if not uploaded_files:
            return Response({"error": "No file uploaded."}, status=status.HTTP_400_BAD_REQUEST)
        if len(uploaded_files) > 1:
            return Response({"error": "Multiple files uploaded."}, status=status.HTTP_400_BAD_REQUEST)
        zip_file = uploaded_files[0]
        if not zip_file.name.lower().endswith(".zip"):
            return Response({"error": "Uploaded file is not a ZIP archive."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            try:
                ...
                return Response({"error": f"Failed to unpack zip: {e}"}, status=status.HTTP_400_BAD_REQUEST)
...
#                             status=status.HTTP_400_BAD_REQUEST)
# 
#         # Retrieve the ProjectInstance early
#         try:
#             instance = ProjectInstance.objects.get(instance_slug=instance_slug)
#         except ProjectInstance.DoesNotExist:
#             return Response(
...
#                     out.write(chunk)
# 
#             # Determine final folder for the unzipped project
#             project_folder_rel = os.path.join("project_files", instance_slug)
#             project_folder_abs = os.path.join(settings.MEDIA_ROOT, project_folder_rel)
# 
#             if os.path.exists(project_folder_abs):
...
#             os.makedirs(files_folder, exist_ok=True)
# 
#             # Convert layers to GeoJSON in the layers_folder if a .gpkg exists
#             collection_path = os.path.join(project_folder_abs, 'collection')
#             gpkg_files = [f for f in os.listdir(collection_path) if f.endswith('.gpkg')]
#             if gpkg_files:
#                 gpkg_path = os.path.join(collection_path, gpkg_files[0])
#                 geojson_output = os.path.join(layers_folder, 'layers.geojson')
#                 subprocess.run([
#                     "ogr2ogr", 
#                     "-f", "GeoJSON", 
#                     "-t_srs", "EPSG:4326", 
#                     geojson_output, gpkg_path
#                 ], check=True)
# 
...
#             )
# 
#             # Update the ProjectInstance with the new folder path
#             instance.qgis_folder_path = project_folder_rel
#             instance.save()
# 
#             return Response(
